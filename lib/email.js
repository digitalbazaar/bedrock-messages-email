  ...
  async.until(function() {return done;}, function(callback) {
    ...
    async.auto({
      ...
      collect: ['pull', function(callback, results) {
        console.log('PULLRESULTS', results);
        if(!results.pull) {
          done = true;
          return callback();
        }
      ...
      identity: ['pull', function(callback, results) {
        ...
        brIdentity.get(null, results.pull.recipient, callback);
      }],
      email: ['collect', 'identity', function(callback, results) {
        ...
        if(results.identity.length > 2) {
          console.log('Recieved multiple identities');
          console.log(results.identity);
          done = true;
          return callback();
        }

        var identity = results.identity[0];
        var messages = results.collect;

        // FIXME: Move the type beging emitted to some kind of config defined higher up
        bedrock.events.emitLater({
          type: 'myProject.emailMessageEvent',
          details: {messages: messages,
                    identity: results.identity[0]}
        });
